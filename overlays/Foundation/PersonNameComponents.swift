...
// From release/5.4 stdlib/public/Darwin/Foundation/PersonNameComponents.swift. Darling changes: phoneticRepresentation
// and _toCustomAnyHashable bridge explicitly (Darling's apinotes don't bridge NSPersonNameComponents), description
// concatenates with + (the overlay has no String.appending), and the macOS 13 memberwise initializer is added.
...
public struct PersonNameComponents : ReferenceConvertible, Hashable, Equatable, _MutableBoxing {
    ...
    public var phoneticRepresentation: PersonNameComponents? {
        get {
            return _handle.map { $0.phoneticRepresentation }.map { PersonNameComponents._unconditionallyBridgeFromObjectiveC($0) }
        }
        set { _applyMutation { $0.phoneticRepresentation = newValue?._bridgeToObjectiveC() } }
    }
...
extension NSPersonNameComponents : _HasCustomAnyHashableRepresentation {
    ...
    public func _toCustomAnyHashable() -> AnyHashable? {
        return AnyHashable(PersonNameComponents._unconditionallyBridgeFromObjectiveC(self))
    }
}
