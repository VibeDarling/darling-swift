...
// Omitted for Darling: AttributedStringProtocol.range(of:options:locale:) and its
// _range helper. Both take a String.CompareOptions default of [], and Darling's
// NSStringCompareOptions imports as a plain raw-value struct rather than an OptionSet
// (see NSStringAPI.swift), so the array literal does not type-check. The body also needs
// swift-foundation's pure-Swift Substring._range(of:options:), which needs the Unicode
// scalar property tables. No binary in the macOS app corpus binds these symbols.
