...
import Darwin
// Darling: no _SwiftFoundationOverlayShims; Darling's NSData has no _isCompact SPI or deallocator globals.

internal func __NSDataIsCompact(_ data: NSData) -> Bool {
    var compact = true
    let len = data.length()
    data.enumerateByteRanges { (_, byteRange, stop) in
...
public struct Data : ReferenceConvertible, Equatable, Hashable, RandomAccessCollection, MutableCollection, RangeReplaceableCollection, MutableDataProtocol, ContiguousBytes {
    ...
    public typealias ReadingOptions = NSDataReadingOptions
    public typealias WritingOptions = NSDataWritingOptions
    public typealias SearchOptions = NSDataSearchOptions
    public typealias Base64EncodingOptions = NSDataBase64EncodingOptions
    public typealias Base64DecodingOptions = NSDataBase64DecodingOptions
    ...
    public enum Deallocator {
        ...
        @usableFromInline internal var _deallocator : ((UnsafeMutableRawPointer, Int) -> Void) {
            ...
            switch self {
            case .virtualMemory:
                return { vm_deallocate(mach_task_self_, vm_address_t(UInt(bitPattern: $0)), vm_size_t($1)) }
            case .unmap:
                return { munmap($0, $1) }
            case .free:
                return { p, _ in Darwin.free(p) }
            case .none:
                return { _, _ in }
            case .custom(let b):
                return b
    ...
    @inlinable // This is @inlinable as a convenience initializer.
    public init?(base64Encoded base64String: __shared String, options: Data.Base64DecodingOptions = Data.Base64DecodingOptions(rawValue: 0)) {
        if let d = NSData(base64EncodedString: base64String, options: Base64DecodingOptions(rawValue: options.rawValue)) {
            self.init(referencing: d)
        } else {
            return nil
        }
    ...
    // MARK: -
#if !DEPLOYMENT_RUNTIME_SWIFT
    private func _shouldUseNonAtomicWriteReimplementation(options: Data.WritingOptions = Data.WritingOptions(rawValue: 0)) -> Bool {
        // Darling: the OS X 10.11 non-atomic write crash workaround doesn't apply.
        return false
    }
#endif
    ...
    public func write(to url: URL, options: Data.WritingOptions = Data.WritingOptions(rawValue: 0)) throws {
    ...
    public func range(of dataToFind: Data, options: Data.SearchOptions = Data.SearchOptions(rawValue: 0), in range: Range<Index>? = nil) -> Range<Index>? {
        let nsRange : NSRange
        if let r = range {
