...
// Adapted for Darling: scanUInt64(representation:) and scanDecimal() are dropped, and the
// remaining scan methods call darling-foundation's selector names. See DARLING-CHANGES.md.
...
// NSCaseInsensitiveSearch, from NSString.h.
private let _caseInsensitiveSearchRawValue: UInt = 1
...
extension Scanner {
    ...
    public func scanInt32(representation: NumberRepresentation = .decimal) -> Int32? {
        return _scan(representation: representation, scanDecimal: self.scanInt(_:), overflowingScanHexadecimal: self.scanHexInt(_:))
    }

    public func scanInt64(representation: NumberRepresentation = .decimal) -> Int64? {
        return _scan(representation: representation, scanDecimal: self.scanLongLong(_:), overflowingScanHexadecimal: self.scanHexLongLong(_:))
    }
    ...
    fileprivate var _currentIndexAfterSkipping: String.Index {
        // darling-foundation's API notes carry no SwiftBridge for NSCharacterSet, so unlike on
        // Apple's SDK this property imports as the class rather than as CharacterSet.
        guard let skips = charactersToBeSkipped as CharacterSet? else { return currentIndex }

        let index = string[currentIndex...].firstIndex(where: { !skips.contains($0) })
        return index ?? string.endIndex
    }

    // Apple's code spells this `self.caseSensitive ? [] : .caseInsensitive` at each call site.
    // Darling's NS_OPTIONS import as a plain raw-value struct rather than an OptionSet, so the
    // option has no Swift member name and no array literal; NSStringAPI.swift in this overlay
    // spells the same constant the same way.
    fileprivate var _compareOptions: String.CompareOptions {
        return String.CompareOptions(rawValue: self.caseSensitive ? 0 : _caseInsensitiveSearchRawValue)
    }
    ...
    public func scanString(_ searchString: String) -> String? {
        ...
        if string.compare(searchString, options: _compareOptions, range: currentIndex ..< substringEnd, locale: self.locale as? Locale) == .orderedSame {
    ...
    public func scanUpToString(_ substring: String) -> String? {
        ...
        repeat {
            guard let range = string.range(of: substring, options: _compareOptions, range: currentSearchIndex ..< string.endIndex, locale: self.locale as? Locale) else {
