...
final class ICUPatternGenerator : @unchecked Sendable {
    ...
    // Darling: upstream asks udatpg_getDefaultHourCycle, which is ICU 67. The hour field that
    // skeleton "j" resolves to is the locale's preferred one, which is what that call reports.
    var defaultHourCycle: Locale.HourCycle {
        guard let pattern = _patternForSkeleton("j") else { return .zeroToTwentyThree }
        var inQuote = false
        for c in pattern {
