...
// From release/5.4 stdlib/public/Darwin/Foundation/IndexSet.swift.
// Darling: NSIndexSet's accessors are methods, the __NSIndexSetRange* overlay shims are implemented in Swift on
// top of -enumerateRangesUsingBlock:, and NSEnumerationOptions is a plain integer.

private func __NSIndexSetRangeCount(_ set: NSIndexSet) -> Int {
    var count = 0
    set.enumerateRanges { _, _ in count += 1 }
    return count
}

private func __NSIndexSetRangeAtIndex(_ set: NSIndexSet, _ rangeIndex: Int, _ location: UnsafeMutablePointer<Int>, _ length: UnsafeMutablePointer<Int>) {
    var i = 0
    location.pointee = NSNotFound
    length.pointee = 0
    set.enumerateRanges { range, stop in
        if i == rangeIndex {
...
private func __NSIndexSetIndexOfRangeContainingIndex(_ set: NSIndexSet, _ index: Int) -> Int {
    var i = 0
...
public struct IndexSet : ReferenceConvertible, Equatable, BidirectionalCollection, SetAlgebra {
    ...
    public func contains(integersIn indexSet: IndexSet) -> Bool {
        return _handle.map { $0.contains(indexSet._bridgeToObjectiveC()) }
    }
    ...
    public func filteredIndexSet(in range : Range<Element>, includeInteger: (Element) throws -> Bool) rethrows -> IndexSet {
        ...
        // Darling's -indexesInRange:options:passingTest: block isn't marked NS_NOESCAPE.
        return try withoutActuallyEscaping(includeInteger) { includeInteger in try _handle.map {
            ...
            if let e = error {
                ...
                return IndexSet._unconditionallyBridgeFromObjectiveC(result)
            }
        } }
    }
...
extension IndexSet {
    public static func ==(lhs: IndexSet, rhs: IndexSet) -> Bool {
        return lhs._handle.map { $0.isEqual(to: rhs._bridgeToObjectiveC()) }
    }
}
