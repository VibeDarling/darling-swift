...
// NSRange, from release/5.4 stdlib/public/Darwin/Foundation/NSRange.swift.
// Darling: -scanInteger: (Darling has no scanInt(_: Int) rename) and the Swift scanUpToCharacters(from:) are used,
// and the removed PlaygroundQuickLook conformance is dropped.
...
extension NSRange {
    public init?(_ string: __shared String) {
        ...
        _ = scanner.scanUpToCharacters(from: digitSet)
        if scanner.isAtEnd {
        ...
        guard scanner.scanInteger(&location) else {
            return nil
        }
        if scanner.isAtEnd {
        ...
        var length = 0
        savedLocation = scanner.scanLocation
        guard scanner.scanInteger(&length) else {
            return nil
        }
