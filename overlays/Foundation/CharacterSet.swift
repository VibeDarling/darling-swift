...
// Darling: the URL*Allowed sets are built from the RFC 3986 character classes (Darling has no
// _CFURLComponentsGetURL*AllowedCharacterSet SPI), and NS_OPTIONS import as raw-value structs.
...
public struct CharacterSet : ReferenceConvertible, Equatable, Hashable, SetAlgebra {
    ...
    public init?(contentsOfFile file: __shared String) {
        do {
            let data = try Data(contentsOf: URL(fileURLWithPath: file), options: Data.ReadingOptions(rawValue: 1) /* mappedIfSafe */)
            _storage = __CharacterSetStorage(immutableReference: CFCharacterSetCreateWithBitmapRepresentation(nil, data as CFData))
        } catch {
            return nil
        }
    ...
    public static var urlUserAllowed : CharacterSet {
        return CharacterSet(charactersIn: "!$&'()*+,-.0123456789;=ABCDEFGHIJKLMNOPQRSTUVWXYZ_abcdefghijklmnopqrstuvwxyz~")
    }
