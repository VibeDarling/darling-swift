...
extension Duration {
    ...
    public struct UnitsFormatStyle : FormatStyle, Sendable {
        ...
        public struct UnitWidth : Codable, Hashable, Sendable {
            // Darling: see _MeasurementUnitWidth at the end of the file.
            var width: _MeasurementUnitWidth
            var patternStyle: UATimeUnitStyle
            
            private init(width: _MeasurementUnitWidth, patternStyle: UATimeUnitStyle) {
                self.width = width
            ...
            public init(from decoder: any Decoder) throws {
                ...
                self.width = try container.decode(_MeasurementUnitWidth.self, forKey: .width)
...
extension Duration.UnitsFormatStyle {
    ...
    public struct Attributed : FormatStyle, Sendable {
        ...
        public func format(_ duration: Duration) -> AttributedString {
            ...
            for formattedField in formattedFields.reversed() {
                // Darling: AttributedString.range(of:options:) is not in this overlay; see DARLING-CHANGES.md.
                let range = result.characters._range(of: "{0}", anchored: false, backwards: true)!
                result.replaceSubrange(range, with: formattedField)
            }
...
// Darling: upstream stores `Measurement<UnitDuration>.FormatStyle.UnitWidth`, from the closed
// Measurement format style. This is the same value as swift-foundation's stub declares it.
struct _MeasurementUnitWidth : Codable, Hashable, Sendable {
