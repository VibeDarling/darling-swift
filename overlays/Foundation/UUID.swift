...
// UUID, from release/5.4 stdlib/public/Darwin/Foundation/UUID.swift.
// Darling: the NSUUID accessors use Darling's selector names, and Comparable (added in macOS 14's Foundation) is
// implemented as a lexicographic comparison of the bytes.
            ...
            $0.withMemoryRebound(to: UInt8.self, capacity: 16) { val in
                withUnsafeMutablePointer(to: &bytes) {
                    $0.withMemoryRebound(to: Int8.self, capacity: 37) { str in
                        ...
                        return String(cString: UnsafePointer(str))
                    }
                }
            }
        }
...
extension UUID : Comparable {
    public static func < (lhs: UUID, rhs: UUID) -> Bool {
        return withUnsafeBytes(of: lhs.uuid) { l in
            withUnsafeBytes(of: rhs.uuid) { r in
                l.lexicographicallyPrecedes(r)
            }
        }
    }
}
