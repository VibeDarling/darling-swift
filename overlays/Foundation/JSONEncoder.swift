...
// JSONEncoder and JSONDecoder, from release/5.4 stdlib/public/Darwin/Foundation/JSONEncoder.swift.
// Darling:
// - Values are written and parsed by _JSONSerialization (JSONSerialization+Swift.swift) instead of Darling's
//   NSJSONSerialization, which ignores sorted keys, pretty-prints differently and has a fragile number parser.
// - The .iso8601 date strategies use a Swift RFC 3339 formatter, since Darling's NSISO8601DateFormatter is a stub.
// - NSNumber is created with Darling's init(integer:)... initializers, booleans are the kCFBooleanTrue/False
//   singletons, and integer unboxing checks exact representability from the number's type encoding.
// - NSMutableDictionary/NSMutableArray are used through -setObject:forKey:, -objectForKey: and -count.
// - There is no Decimal overlay, so Decimal/NSDecimalNumber values are not special-cased.
...
open class JSONEncoder {
    ...
    public enum KeyEncodingStrategy {
        ...
        fileprivate static func _convertToSnakeCase(_ stringKey: String) -> String {
            ...
            while let upperCaseRange = stringKey.rangeOfCharacter(from: CharacterSet.uppercaseLetters, options: String.CompareOptions(rawValue: 0), range: searchRange) {
                let untilUpperCase = wordStart..<upperCaseRange.lowerBound
                words.append(untilUpperCase)
                
                // Find next lowercase character
                searchRange = upperCaseRange.lowerBound..<searchRange.upperBound
                guard let lowerCaseRange = stringKey.rangeOfCharacter(from: CharacterSet.lowercaseLetters, options: String.CompareOptions(rawValue: 0), range: searchRange) else {
    ...
    open func encode<T : Encodable>(_ value: T) throws -> Data {
        ...
        var writingOptions = _JSONSerialization.WritingOptions()
        writingOptions.prettyPrinted = self.outputFormatting.contains(.prettyPrinted)
        writingOptions.sortedKeys = self.outputFormatting.contains(.sortedKeys)
        writingOptions.withoutEscapingSlashes = self.outputFormatting.contains(.withoutEscapingSlashes)
        do {
           return try _JSONSerialization.data(with: topLevel, options: writingOptions)
        } catch {
            throw EncodingError.invalidValue(value, 
...
private struct _JSONKeyedEncodingContainer<K : CodingKey> : KeyedEncodingContainerProtocol {
    ...
    public mutating func encodeNil(forKey key: Key) throws {
        self.container.setObject(NSNull(), forKey: _converted(key).stringValue as NSString)
    }
    public mutating func encode(_ value: Bool, forKey key: Key) throws {
        self.container.setObject(self.encoder.box(value), forKey: _converted(key).stringValue as NSString)
    }
    ...
    public mutating func encode(_ value: Float, forKey key: Key) throws {
        ...
        self.container.setObject(try self.encoder.box(value), forKey: _converted(key).stringValue as NSString)
    }
    ...
    public mutating func nestedContainer<NestedKey>(keyedBy keyType: NestedKey.Type, forKey key: Key) -> KeyedEncodingContainer<NestedKey> {
        ...
        let dictionary: NSMutableDictionary
        if let existingContainer = self.container.object(forKey: containerKey as NSString) {
            precondition(
            ...
            self.container.setObject(dictionary, forKey: containerKey as NSString)
        }
    ...
    public mutating func nestedUnkeyedContainer(forKey key: Key) -> UnkeyedEncodingContainer {
        ...
        if let existingContainer = self.container.object(forKey: containerKey as NSString) {
            ...
            self.container.setObject(array, forKey: containerKey as NSString)
        }
    ...
    func box(_ date: Date) throws -> NSObject {
        ...
        case .iso8601:
            guard let string = _ISO8601.string(from: date) else {
                throw EncodingError.invalidValue(date, EncodingError.Context(codingPath: self.codingPath, debugDescription: "Date \(date.timeIntervalSince1970) can't be formatted as ISO 8601."))
            }
    ...
    func box(_ dict: [String : Encodable]) throws -> NSObject? {
        ...
        do {
            for (key, value) in dict {
                ...
                defer { self.codingPath.removeLast() }
                result.setObject(try box(value), forKey: key as NSString)
            }
        } catch {
            // If the value pushed a container before throwing, pop it back off to restore state.
            if self.storage.count > depth {
...
open class JSONDecoder {
    ...
    open func decode<T : Decodable>(_ type: T.Type, from data: Data) throws -> T {
        let topLevel: Any
        do {
           topLevel = try _JSONSerialization.object(with: data)
        } catch {
            // JSONSerialization reports NSCocoaErrorDomain code 3840 (NSPropertyListReadCorruptError) on macOS.
            let underlying = NSError(domain: "NSCocoaErrorDomain", code: 3840, userInfo: ["NSDebugDescription": "\(error)"])! as Error
            throw DecodingError.dataCorrupted(DecodingError.Context(codingPath: [], debugDescription: "The given data was not valid JSON.", underlyingError: underlying))
        }
...
private extension __JSONDecoder {
    ...
    func unbox(_ value: Any, as type: Bool.Type) throws -> Bool? {
        ...
        if let number = value as? NSNumber {
            ...
            if number === _JSONSerialization.trueNumber {
                return true
            } else if number === _JSONSerialization.falseNumber {
                return false
            }
    ...
    func unbox(_ value: Any, as type: Int.Type) throws -> Int? {
        ...
        if let big = value as? _JSONUnsignedNumber {
            guard let result = type.init(exactly: big.value) else {
                throw DecodingError.dataCorrupted(DecodingError.Context(codingPath: self.codingPath, debugDescription: "Parsed JSON number <\(big.value)> does not fit in \(type)."))
            }
            return result
        }

        guard let number = value as? NSNumber, number !== _JSONSerialization.trueNumber, number !== _JSONSerialization.falseNumber else {
            throw DecodingError._typeMismatch(at: self.codingPath, expectation: type, reality: value)
        }
    ...
    func unbox(_ value: Any, as type: Float.Type) throws -> Float? {
        ...
        if let big = value as? _JSONUnsignedNumber { return Float(big.value) }

        if let number = value as? NSNumber, number !== _JSONSerialization.trueNumber, number !== _JSONSerialization.falseNumber {
    ...
    func unbox(_ value: Any, as type: Double.Type) throws -> Double? {
        ...
        if let big = value as? _JSONUnsignedNumber { return Double(big.value) }
    ...
    func unbox(_ value: Any, as type: Date.Type) throws -> Date? {
        ...
        case .iso8601:
            ...
            guard let date = _ISO8601.date(from: string) else {
    ...
    func unbox<T>(_ value: Any, as type: _JSONStringDictionaryDecodableMarker.Type) throws -> T? {
        ...
        for key in dict.allKeys() {
            let value: Any = dict.object(forKey: key)!
...
//===----------------------------------------------------------------------===//
// Shared ISO8601 Date Formatting
//===----------------------------------------------------------------------===//

/// RFC 3339 internet date-times ("2021-07-15T12:00:00Z"), which ISO8601DateFormatter produces with
/// `.withInternetDateTime`. Darling's NSISO8601DateFormatter is an empty stub, so this works in UTC arithmetic.
private enum _ISO8601 {
    ...
    /// Nil for dates outside the years 0000-9999, which `date(from:)` couldn't read back (or not finite).
    static func string(from date: Date) -> String? {
        let interval = date.timeIntervalSince1970.rounded(.down)
        guard interval.isFinite, abs(interval) < 1e14 else { return nil }
        let seconds = Int(interval)
        let days = seconds >= 0 ? seconds / 86400 : (seconds - 86399) / 86400
        let secondOfDay = seconds - days * 86400
        let (year, month, day) = civil(days: days)
        guard (0...9999).contains(year) else { return nil }
        return "\(pad(year, 4))-\(pad(month, 2))-\(pad(day, 2))T\(pad(secondOfDay / 3600, 2)):\(pad(secondOfDay / 60 % 60, 2)):\(pad(secondOfDay % 60, 2))Z"
    }

    /// Parses "yyyy-MM-ddTHH:mm:ss" followed by "Z" or a "+HH:MM"/"+HHMM" offset.
    static func date(from string: String) -> Date? {
        ...
        func number(_ range: Range<Int>) -> Int? {
            ...
            var value = 0
            for index in range {
                guard (UInt8(ascii: "0")...UInt8(ascii: "9")).contains(chars[index]) else { return nil }
                value = value * 10 + Int(chars[index] - UInt8(ascii: "0"))
            }
            return value
        }
        guard chars.count >= 20,
              chars[4] == UInt8(ascii: "-"), chars[7] == UInt8(ascii: "-"),
              chars[10] == UInt8(ascii: "T"),
              chars[13] == UInt8(ascii: ":"), chars[16] == UInt8(ascii: ":"),
              ...
              (1...12).contains(month), day >= 1, hour < 24, minute < 60, second < 60 else {
            return nil
        }
        let offset: Int
        let zone = Array(chars[19...])
        if zone == [UInt8(ascii: "Z")] {
            offset = 0
        } else if zone.first == UInt8(ascii: "+") || zone.first == UInt8(ascii: "-") {
            let minuteStart: Int
            if zone.count == 6 && zone[3] == UInt8(ascii: ":") {
                minuteStart = 23
            ...
            guard let offsetHours = number(20..<22), let offsetMinutes = number(minuteStart..<minuteStart + 2), offsetHours < 24, offsetMinutes < 60 else {
                return nil
            }
            offset = (zone.first == UInt8(ascii: "-") ? -1 : 1) * (offsetHours * 3600 + offsetMinutes * 60)
        } else {
            return nil
        }
        let days = self.days(year: year, month: month, day: day)
